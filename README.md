...
1. **董監事資料空白**：改用 GCIS 董監事 API 實際回傳欄位：`Person_Position_Name`、`Person_Name`、`Juristic_Person_Name`、`Person_Shareholding`。
2. **關聯圖加入後沒有變動**：修正 Canvas 在隱藏區塊初始化時寬高為 0 的問題，加入公司前會重新 resize。
3. **自動展開關聯**：新增「自動展開關聯」按鈕，會針對代表人、董監事名單、分公司經理人做公開 API 可查範圍的擴大搜尋。

### 重要限制

目前 GCIS 公開 API 可直接做到：
...
但公開 API **沒有提供「用人名直接查所有公司董監事／經理人」的開放端點**。若要完整達成供應商 DD 的人名反查，建議做法是下載「公司登記董監事資料」CSV，在後端或本地 IndexedDB 建立 `Person_Name -> 公司統編` 的反向索引。
